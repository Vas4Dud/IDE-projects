 ...
 * ******************************************************************************
 * @file    : oled.c
 * @brief   : OLED module operations
 * @details : Provides OLED initialization and additional functions
 * @note    : Requires implemented I2C module driver
 * 
 * @author RAJ JOHRI
...
static unsigned char num_str[95][16] = {
...
int px_height;
    ...
    for(i = 0;i < 128;i++)
    {
        ...
        px_height = 64 - (uint16_t) (floor((camera_in_array[i] * 64.0) / 0x3FFF));
        if (px_height > 63)
            px_height = 63;
...
void OLED_PrintBlock(int startRow, int startCol, char *lines[], int numLines)
{
    ...
    for(i = 0; i < numLines; i++) {
        if(lines[i] != NULL) {
            OLED_draw_line(startRow + i, startCol, (unsigned char *)lines[i]);
        }
    }
    OLED_write_display(OLED_TEXT_ARR);
}

void OLED_Print(int row, int col, char *txt)
{
    char *lines[1] = { txt };
    OLED_PrintBlock(row, col, lines, 1);
}
